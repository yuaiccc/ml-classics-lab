...
- **`rl-lab/`** —— 一个 React + Vite 前端「**算法过程动画实验台**」（主页即是）。左侧是一棵 **AI 发展脉络树**（分支=方法谱系，叶子=实验，带年代），33 个实验（含真实数据 Iris、评估泛化、本地 Qwen 嵌入与 Agent、监督/无监督/深度学习/生成模型/强化学习）统一在一个时间轴播放器里边看边学。
...
`/lab` 共 **33 个实验**（含真实数据 Iris、评估泛化、本地 Qwen 大模型嵌入与 Agent），按 AI 发展史排开——从 **1958 感知机** 一路到 **2017 Transformer、2020 Diffusion、2023 Mamba**，覆盖符号/统计学习、联想记忆、深度学习、生成模型、强化学习几乎所有里程碑。其中 8 个里程碑实验（SVM / AdaBoost / Hopfield / RNN / Word2Vec / GAN / Diffusion / Q-Learning）全部浏览器端手写、含真实训练、零 ML 库。
