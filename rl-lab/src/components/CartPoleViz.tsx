...
const EPOCH_CONFIGS = [
  { label: "Epoch 0 (随机)", reward: "~22", color: "#ff5252" },
  { label: "Epoch 1 (初学)", reward: "~173", color: "#ffab40" },
  { label: "Epoch 2 (进步)", reward: "~304", color: "#00e5ff" },
  { label: "Epoch 3 (接近)", reward: "~444", color: "#00e5ff" },
  { label: "Epoch 4 (完美)", reward: "~500", color: "#00ff88" },
];
  ...
  return (
    <div className="glass rounded-xl p-6">
        ...
        {EPOCH_CONFIGS.map((cfg, i) => (
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-[10px] font-mono border transition-all duration-200 ${
              epochIdx === i
                ? "border-opacity-60"
                : "border-slate-700/50 text-slate-600 hover:border-slate-600"
            }`}
            style={
              epochIdx === i
                ? {
      ...
      <div className="flex items-center justify-center gap-4 text-xs text-slate-500">
        <span>
          步数:{" "}
          <span className="font-mono text-slate-300">{stepCount}</span>
        </span>
        <span>
          状态:{" "}
          <span
