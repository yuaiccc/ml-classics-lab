...
export default function ExperimentCard({ experiment }: { experiment: Experiment }) {
  ...
  const color = CURVE_COLORS[experiment.algorithm] || "#00ff88";
  ...
  return (
    <div className="glass glass-hover rounded-xl p-5 transition-all duration-300 group cursor-pointer h-full flex flex-col">
      ...
      <h3 className="text-base font-bold text-slate-200 mb-1 font-mono">
        {experiment.env}
      </h3>
      ...
      <div className="flex items-center justify-between text-xs text-slate-500">
        ...
        <ArrowRight className="w-3 h-3 text-slate-600 group-hover:text-[#00ff88] transition-colors" />
      </div>
    </div>
  );
}
