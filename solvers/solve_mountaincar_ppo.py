...
def main() -> None:
    ...
    maybe_record(
        ...
        meta={
            ...
            "tutorial": {
                "problem": "稀疏奖励难题：小车动力不足，无法直接冲上山顶，必须先后退借力。",
                "intuition": "每步奖励都是 -1，唯一的正反馈在山顶——随机策略几乎碰不到。PPO 靠随机策略+熵正则持续探索，偶然到顶后，迅速学会“先退后冲”这个反直觉的借力策略。",
                "watch": [
                    "小车不是直接冲右，而是先往左退、积累动量",
                    "再借势冲向右上方，到达旗帜时回放结束",
                    "Return = 步数的负值，用的步数越少越好",
                ],
                "concepts": [
                    {"term": "稀疏奖励", "explain": "有用的正反馈极少出现，智能体很难学到"},
                    {"term": "探索", "explain": "主动尝试没见过的动作，才可能撞见山顶的正信号"},
                    {"term": "借力策略", "explain": "先反方向蓄能、再顺势冲——非直觉但最优"},
                ],
                "tryThis": "看小车开局往哪边走——会发现它先退后冲，而不是硬冲右边。",
            },
